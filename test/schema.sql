...
-- CREATE DATABASE signaturestore WITH TEMPLATE = template0
...
CREATE INDEX library_resource ON public.libraries USING btree
(((meta ->> 'Primary Resource'::text)));
...
CREATE INDEX meta_tsvector_idx ON public.signatures USING gin
(meta_tsvector);
...
CREATE INDEX signature_assay ON public.signatures USING btree
(((meta ->> 'Assay'::text)));
...
CREATE INDEX signature_organism ON public.signatures USING btree
(((meta ->> 'Organism'::text)));
...
CREATE INDEX signature_perturbation ON public.signatures USING btree
(((meta ->> 'Perturbation Type'::text)));
...
CREATE INDEX signatures_fkey_library ON public.signatures USING btree
(libid);
...
CREATE INDEX signatures_to_tsvector_idx ON public.signatures USING gin
(to_tsvector
('english'::regconfig,
(meta)::text));
...
CREATE INDEX signatures_to_tsvector_idx1 ON public.signatures USING gin
(to_tsvector
('english'::regconfig, meta));
...
CREATE INDEX signatures_to_tsvector_idx2 ON public.signatures USING gist
(to_tsvector
('english'::regconfig,
(meta)::text));
