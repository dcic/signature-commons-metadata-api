...
import {Json, JsonObject} from '../types/schemas';
...
export function findJoin<T>(
  ...
  if (metadata.isOneToOne || metadata.isOneToMany || metadata.isManyToOne) {
    if (metadata.isOwning) {
      ...
      asExpression = (leftAlias, rightAlias, joinKind = 'inner join') => {
        if (metadata.inverseRelation === undefined)
          throw new Error('Unhandled');
        return `${joinKind} "${
          metadata.inverseEntityMetadata.tableName
    ...
    if (metadata.isOwning) {
      asExpression = (leftAlias, rightAlias, joinKind = 'inner join') => {
        ...
        if (metadata.junctionEntityMetadata === undefined)
          throw new Error('Unhandled');
        return `
          ${joinKind} "${
      ...
      asExpression = (leftAlias, rightAlias, joinKind = 'inner join') => {
        const junctionAlias = `${leftAlias}__${rightAlias}`;
        if (
          metadata.inverseRelation === undefined ||
          metadata.inverseRelation.junctionEntityMetadata === undefined
        )
          throw new Error('Unhandled');
        return `
          ${joinKind} "${
