...
CONTAINER_STATUS=$(docker run --name $POSTGRESQL_CONTAINER -e POSTGRES_USER=$USER -e POSTGRES_PASSWORD=$PASSWORD -p $PORT:5432 -e POSTGRES_DB=$DATABASE -d postgres:latest 2>&1)
...
export POSTGRESQL_TEST_URL="posgresql://${USER}:${PASSWORD}@${HOST}:${PORT}/${DATABASE}"
